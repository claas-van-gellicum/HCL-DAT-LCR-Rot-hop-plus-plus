...
def main():
    ...
    # Name, year of source, name, year of tpytharget, corresponding batch sizes as defined in main_test
...
def run_hyper(source_domain, source_year, target_domain, target_year, batch_size_src, batch_size_tar, batch_size_te,
              runs, n_iter):
    """
    Method obtained from Knoester et al. (2023)
    ...
    #CL-XD-ABSA extension: If tuning for base model, set lambda's to 0.
    #CL-XD-ABSA extension: If tuning for CL extension, set hyperparameters of base model and tune CL parameters.
    ...
    CLRH_space = [
        hp.choice('learning_rate_dis', [0.005]),
        hp.choice('learning_rate_f', [0.005]),
        hp.choice('keep_prob', [0.30]),
        hp.choice('momentum_dis', [0.8]),
        hp.choice('momentum_f', [0.9]),
        hp.choice('l2_dis', [0.001]),
        hp.choice('l2_f', [0.0001]),
        # hp.choice('balance_lambda', [0.6]),
        ...
        hp.choice('tau_d', [0.01, 0.05, 0.07, 0.1, 0.2]),
        hp.choice('tau_c', [0.01, 0.05, 0.07, 0.1, 0.2]),
        hp.choice('lambda_dcl', [0.05, 0.1, 0.2, 0.5, 0.7]),
        hp.choice('lambda_ccl', [0.05, 0.1, 0.2, 0.5, 0.7]),
    ]
...
def CLRH_objective(hyperparams, path):
    ...
    eval_num += 1
    (learning_rate_dis, learning_rate_f, keep_prob, momentum_dis, momentum_f, l2_dis, l2_f, balance_lambda, tau_d, tau_c, lambda_dcl, lambda_ccl) = hyperparams
    print("Current hyperparameters: " + str(hyperparams))

    l, pred1, fw1, bw1, tl1, tr1 = HCL_DAT_LCR_Rot_hop.main(FLAGS.hyper_train_path_source, FLAGS.hyper_train_path_target,
                                            FLAGS.hyper_eval_path_target, learning_rate_dis, learning_rate_f,
                                            keep_prob, momentum_dis, momentum_f, l2_dis, l2_f, balance_lambda, tau_d, tau_c, lambda_dcl, lambda_ccl)
